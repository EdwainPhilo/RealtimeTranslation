                ...
                for rate in sample_rates_to_try:
                    try:
                        ...
                        if stream is not None:
                            logging.debug(
                                f"Audio recording initialized successfully at {device_sample_rate} Hz, reading {chunk_size} frames at a time")
            ...
            try:
                ...
                if status == 'success':
                    ...
                    self.last_transcription_bytes_b64 = base64.b64encode(
                        self.last_transcription_bytes.tobytes()).decode('utf-8')
                ...
                if not self.is_recording:
                    ...
                    if self.stop_recording_on_voice_deactivity:
                        ...
                        if not self.speech_end_silence_start:
                            ...
                            str_speech_end_silence_start = datetime.datetime.fromtimestamp(
                                self.speech_end_silence_start).strftime('%H:%M:%S.%f')[:-3]
                        if self.use_extended_logging:
                            logging.debug(
                        ...
                        if not is_speech:
                            ...
                            if self.speech_end_silence_start and self.early_transcription_on_silence and len(
                                    self.frames) > 0 and \
                                    (
